...
from dataloader.dataset_folder import ImageFolder, ImageWithFixedHint, ImageWithFixedHintAndCoord, TrainDataset, ValDataset
from dataloader.hint_generator import InteractiveHintGenerator, RandomHintGenerator
...
class DataTransformationFixedHint:
    ...
    def __call__(self, img, hint_coords):
        # print(self.img_transform(img).shape)
...
# for training (albumentation)
class DataAugmentationForIColoriT2:
    def __init__(self, args):
        # No normalization on RGB space
        self.transform = A.Compose([
            ...
            # A.GaussNoise(p=0.4),
            # A.JpegCompression (quality_lower=95, p=0.1),
            A.Normalize(
                mean=[0, 0, 0],
                std=[1, 1, 1],
            ),
    ...
    def __call__(self, image):
        return self.transform(image=image)['image'], self.hint_generator()
...
# validatoin, test (albumentation)
class DataTransformationFixedHint2:
    def __init__(self, args) -> None:
        self.input_size = args.input_size
        self.hint_size = args.hint_size
    ...
    def __call__(self, img, hint_coords):
        # print(self.img_transform(img).shape)
        return self.img_transform(image=img)['image'], self.coord2hint(hint_coords)
...
def build_pretraining_dataset(args):
    transform = DataAugmentationForIColoriT2(args)
    print("Data Aug = %s" % str(transform))
    return TrainDataset(args.data_path, transform=transform)

# use for validation, test
def build_fixed_validation_dataset(args):
    transform = DataTransformationFixedHint2(args)
    print("Data Trans = %s" % str(transform))
    return ValDataset(args.val_data_path, args.hint_dirs, transform=transform,
                              return_name=args.return_name, gray_file_list_txt=args.gray_file_list_txt)

#######3
# use for train
def build_pretraining_dataset2(args):
...
# use for validation, test
def build_fixed_validation_dataset2(args):
