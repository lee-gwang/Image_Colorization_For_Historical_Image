...
#         print('H, W, wins_size', H, W, win_size)
...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans=4, dd_in=2,
        ...
        self.output_proj = OutputProj(in_channel=2*embed_dim, out_channel=dd_in, kernel_size=3, stride=1)
    ...
    # def forward(self, x, mask=None):
        ...
        y = self.pos_drop(y)
        #Encoder
        conv0 = self.encoderlayer_0(y,mask=None)
        pool0 = self.dowsample_0(conv0)
        conv1 = self.encoderlayer_1(pool0,mask=None)
        pool1 = self.dowsample_1(conv1)
        conv2 = self.encoderlayer_2(pool1,mask=None)
        pool2 = self.dowsample_2(conv2)
        conv3 = self.encoderlayer_3(pool2,mask=None)
        pool3 = self.dowsample_3(conv3)

        # Bottleneck
        conv4 = self.conv(pool3, mask=None)

        #Decoder
        up0 = self.upsample_0(conv4)
        deconv0 = torch.cat([up0,conv3],-1)
        deconv0 = self.decoderlayer_0(deconv0,mask=None)
        
        up1 = self.upsample_1(deconv0)
        deconv1 = torch.cat([up1,conv2],-1)
        deconv1 = self.decoderlayer_1(deconv1,mask=None)

        up2 = self.upsample_2(deconv1)
        deconv2 = torch.cat([up2,conv1],-1)
        deconv2 = self.decoderlayer_2(deconv2,mask=None)

        up3 = self.upsample_3(deconv2)
        deconv3 = torch.cat([up3,conv0],-1)
        deconv3 = self.decoderlayer_3(deconv3,mask=None)
...
@register_model
def ijcai_uformer_tiny(pretrained=False, **kwargs):
    depths=[2, 2, 2, 2, 2, 2, 2, 2, 2]
    input_size = 224

    model = Uformer(img_size=input_size, embed_dim=4, depths=depths,
                 win_size=7, mlp_ratio=4., token_projection='linear', token_mlp='leff', modulator=True, shift_flag=False)
    return model
@register_model
def ijcai_uformer_small(pretrained=False, **kwargs):
    depths=[2, 2, 2, 2, 2, 2, 2, 2, 2]
    input_size = 224

    model = Uformer(img_size=input_size, embed_dim=8, depths=depths,
                 win_size=7, mlp_ratio=4., token_projection='linear', token_mlp='leff', modulator=True, shift_flag=False)
    return model

@register_model
def ijcai_uformer_base(pretrained=False, **kwargs):
    depths=[2, 2, 2, 2, 2, 2, 2, 2, 2]
    input_size = 224

    model = Uformer(img_size=input_size, embed_dim=10, depths=depths,
                 win_size=7, mlp_ratio=2., token_projection='linear', token_mlp='leff', modulator=True, shift_flag=False)
    return model