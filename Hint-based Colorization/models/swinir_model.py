...
#     print('H, W, window', H, W, window_size)
    return windows
...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=4,
        ...
        # self.conv_first = nn.Conv2d(num_in_ch, embed_dim, kernel_size=patch_size, stride=patch_size)
#         self.conv_first = nn.Conv2d(num_in_ch, embed_dim, 3, 1, 1)
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=num_in_ch, embed_dim=embed_dim,
            norm_layer=norm_layer if self.patch_norm else None)
        num_patches = self.patch_embed.num_patches
        patches_resolution = self.patch_embed.patches_resolution
        self.patches_resolution = patches_resolution
        print('patch resolution', patches_resolution)
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.conv_last = nn.Conv2d(num_out_ch, 2, 3, 1, 1)
            ...
            self.conv_last = nn.Conv2d(num_feat, num_out_ch, 2, 1, 1)
    ...
    def flops(self):
        ...
        return flops
@register_model
def ijcai_swinir_super_tiny(pretrained=False, **kwargs):
    height, width = 224, 224
    window_size = 7
    upscale = 16
    patch_size = upscale

    model = SwinIR(upscale=upscale, img_size=(height, width), patch_size = patch_size,
               window_size=window_size, img_range=1., depths=[6, 6, 6, 6],
               embed_dim=108, num_heads=[6, 6, 6, 6], mlp_ratio=2, upsampler='pixelshuffledirect') # 60
    return model

@register_model
def ijcai_swinir_tiny(pretrained=False, **kwargs):
    height, width = 224, 224
    window_size = 7
    upscale = 16
    patch_size = upscale
    ...
    model = SwinIR(upscale=upscale, img_size=(height, width), patch_size = patch_size,
               window_size=window_size, img_range=1., depths=[6, 6, 6, 6],
               embed_dim=180, num_heads=[6, 6, 6, 6], mlp_ratio=2, upsampler='pixelshuffledirect') # 60
    return model

@register_model
def ijcai_swinir_small(pretrained=False, **kwargs):
    height, width = 224, 224
    window_size = 7
    upscale = 8
    patch_size = upscale
    ...
    model = SwinIR(upscale=upscale, img_size=(height, width), patch_size = patch_size,
               window_size=window_size, img_range=1., depths=[6, 6, 6, 6],
               embed_dim=180, num_heads=[6, 6, 6, 6], mlp_ratio=2, upsampler='pixelshuffledirect')
    return model
