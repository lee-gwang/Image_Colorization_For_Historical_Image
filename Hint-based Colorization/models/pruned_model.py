...
"""잘못한거"""
class BlockLayerSelect2(nn.Module):
...
class BlockLayerSelect(nn.Module):
    def __init__(self, dim_in, num_sub_layer, tau=5, is_hard=True, threshold=0.5, bias=True):
        super().__init__()
        self.mlp_head = nn.Linear(dim_in, 2, bias=bias) # Attention/MLP
        # self.norm = LayerNorm(dim_in)
        self.is_hard = is_hard
        self.tau = tau
        self.threshold = threshold
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0.,
        ...
        if use_rpb:
            ...
            self.rpb_table = nn.Parameter(torch.zeros((2 * window_size - 1) * (2 * window_size - 1), num_heads)) # [729, 3])
...
class Block2(nn.Module):
    ...
    def forward(self, x, layer_mask):
        # x shape : (BS, L, C)
        # layer_mask shape : (BS, 1)
        if self.gamma_1 is None:
            x = x + self.drop_path(self.attn(self.norm1(x)))
            x = x + layer_mask[:, :, None] * self.drop_path(self.mlp(self.norm2(x)))
        else:
            x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x)))
            x = x + layer_mask[:, :, None] * self.drop_path(self.gamma_2 * self.mlp(self.norm2(x)))
        return x
...
class Block(nn.Module):
    ...
    def forward(self, x):
        # x shape : (BS, L, C)
        # layer_mask shape : (BS, 2)
        policy_token = x[:, 0] # (BS, C)
        layer_mask = self.layer_select(policy_token) # (BS, 2)
        if self.gamma_1 is None:
            
            # # attention
            # if (layer_mask[:, 0]==0)[0] == True:
            #     pass
            # else:
            #     x = x + layer_mask[:, 0][:, None, None] * self.drop_path(self.attn(self.norm1(x)))
            # # mlp
            # if (layer_mask[:, 1]==0)[0] == True:
            #     pass
            # else:
            #     x = x + layer_mask[:, 1][:, None, None] * self.drop_path(self.mlp(self.norm2(x)))

            x = x + layer_mask[:, 0][:, None, None] * self.drop_path(self.attn(self.norm1(x)))
            x = x + layer_mask[:, 1][:, None, None] * self.drop_path(self.mlp(self.norm2(x)))
        else:
            x = x + layer_mask[:, 0][:, None, None] * self.drop_path(self.gamma_1 * self.attn(self.norm1(x)))
            x = x + layer_mask[:, 1][:, None, None] * self.drop_path(self.gamma_2 * self.mlp(self.norm2(x)))
        return x, layer_mask
...
class IColoriT2(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=512, embed_dim=512, depth=12,
        ...
        self.pos_embed = get_sinusoid_encoding_table(num_patches+1, embed_dim) # with policy token
        ...
        # related pruned
        self.layer_select = BlockLayerSelect(embed_dim, 1)
        self.policy_token = nn.Parameter(torch.zeros(1, 1, embed_dim)) # BS, L, C
        trunc_normal_(self.policy_token, std=.02)
    ...
    def forward_features(self, x, mask):
        ...
        #### gwang add
        # policy_token
        policy_token = self.policy_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        x = torch.cat((policy_token, x), dim=1)
        ...
        policy_token = x[:,0] # bs, c
        selected_mask = self.layer_select(policy_token) # (BS, 2)
        mask_list.append(selected_mask)

        for blk in self.blocks:
            x = blk(x, selected_mask)
            policy_token = x[:,0] # bs, c
            selected_mask = self.layer_select(policy_token) # (BS, 1)
            mask_list.append(selected_mask)

        x = self.norm(x)
        return x, torch.stack(mask_list, dim=1) # (BS, n_layers, 1)

    def forward(self, x, mask):
        x, mask_list = self.forward_features(x, mask)
        x = x[:,1:]
        x = self.head(x)
...
class IColoriT(nn.Module):
    ...
    def forward_features(self, x, mask):
        ...
        x = self.norm(x)
        return x, torch.stack(mask_list, dim=1) # (BS, n_layers, 2)
...
@register_model
def block_icolorit_tiny_4ch_patch16_224(pretrained=False, **kwargs):
    model = IColoriT(
        num_classes=512,
        img_size=224,
        patch_size=16,
...
@register_model
def block_icolorit_small_4ch_patch16_224(pretrained=False, **kwargs):
    model = IColoriT(
        img_size=224,
        patch_size=16,
        in_chans=4,
...
@register_model
def block_icolorit_base_4ch_patch16_224(pretrained=False, **kwargs):
    model = IColoriT(
        num_classes=512,
        img_size=224,
        patch_size=16,
