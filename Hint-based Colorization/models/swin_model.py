...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 fused_window_process=False, pretrained_cfg=None):
        super().__init__()
        self.dim = dim
...
class CnnHead(nn.Module):
    def __init__(self, embed_dim, num_classes, window_size):
        ...
        self.head = nn.Conv2d(embed_dim, num_classes, kernel_size=3, stride=1, padding=1, groups=self.groups, padding_mode='reflect')

        # for calculate the flops
        self.embed_dim = embed_dim
        self.num_classes = num_classes
    def forward(self, x):
    ...
    def flops(self, x):
        bs, L, _ = x.shape
        return ((3*3) * L * self.embed_dim * self.num_classes) * bs /1e+9, ((3*3) * L * self.embed_dim * self.num_classes) * bs /self.groups /1e+9
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, fused_window_process=False, avg_hint=False, **kwargs):
        super().__init__()
        ...
        # for col
        self.head = CnnHead(embed_dim*8, num_classes, window_size=img_size // 32)
        self.tanh = nn.Tanh()
    ...
    def forward(self, x, mask):
        ...
        x = self.head(x)
        x = self.tanh(x)
        # x = rearrange(x, 'b (h w) (p1 p2 c) -> b c (h p1) (w p2)', p1=32 , p2=32, h=self.img_size//32, w=self.img_size//32)
...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 12],
                 window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False, fused_window_process=False, avg_hint=False, **kwargs):
        ...
        for i_layer in range(self.num_layers):
            ...
            pr = patches_resolution[0] // (2 ** i_layer)
            if i_layer==3:
                dim = int(embed_dim * 2 ** 2)
                pr = patches_resolution[0] // (2 ** 2)
            layer = BasicLayer(dim=dim,
                               input_resolution=(pr, pr),
                               depth=depths[i_layer],
                               num_heads=num_heads[i_layer],
                               window_size=window_size,
                               mlp_ratio=self.mlp_ratio,
        ...
        # for col
        self.head = CnnHead(embed_dim*4, num_classes, window_size=img_size // 16)
        self.tanh = nn.Tanh()
        self.avg_hint = avg_hint
        self.in_chans = in_chans
...
@register_model
def swin_transormer(pretrained=False, **kwargs):
...
@register_model
def swin_transormer_v2(pretrained=False, **kwargs):
    # downsampling 횟수 변경
...
@register_model
def swin_transormer_small_v2(pretrained=False, **kwargs):
    model = SwinTransformerV2(
        num_classes=512,
        img_size=224,
        in_chans=4,
...
@register_model
def swin_base_patch4_window7_224_22k(pretrained=False, **kwargs):
    model = SwinTransformer(
        num_classes=2048,
        img_size=224,
        in_chans=4,
