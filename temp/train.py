...
class CFG:
    ...
    wandb         = False
    exp_name      = 'imagenet_sample30' #[2.5D, 5D_1S]
    model_name    = ['Unet'] # decoder
    backbone      = [ 'efficientnet-b4'] # encoder # LeViT_UNet_384 #efficientnet-b2 # 'se_resnext50_32x4d'
    add_comment   = f'{exp_name}-sample30-new'#'negative-5k-bs32'
    ...
    folds         = [0]
    gpu           = '0,1'
    save_img      = True #True/False # save valid predict images
...
def run_training(model, df, run, device, fold, LOGGER):
    ...
    train_dataset = BuildDataset(train_df, type='train')
    valid_dataset = BuildDataset(valid_df, type='valid')
    ...
    for epoch in range(1, CFG.epochs + 1): 
        ...
        if CFG.save_img:
            ...
            for n, show_img in enumerate(y_pred):
                show_img = cv2.cvtColor(show_img, cv2.COLOR_LAB2BGR)
